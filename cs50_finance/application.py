...
from utility import execute, strategy1, strategy2, strategy3
import pandas as pd
...
df = pd.read_csv('./data/sp500_joined_closes.csv', parse_dates = True, index_col = 0)
...
def strats():
    ...
    # Simple 100-day moving average strategy on AAPL, upon manual execution of MA100-cross-over strategy
    if request.method == "POST":
        #execute(strategy1(df))
        #execute(strategy2(df))
        execute(strategy3(df))
